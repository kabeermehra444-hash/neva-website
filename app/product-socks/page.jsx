...
export default function ProductSocksPage() {
  const router = useRouter();
  const [selectedSize, setSelectedSize] = useState('M');
  const [mainImage, setMainImage] = useState('https://images.unsplash.com/photo-1549421263-50432c7457c4?q=80&w=1000&auto=format&fit=crop');
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    ...
                    <div>
                        <div className="aspect-square bg-gray-100 mb-4 overflow-hidden rounded-lg">
                            <img src={mainImage} alt="Tournament Socks" className="w-full h-full object-cover" />
                        </div>
                        {/* Thumbnail Gallery */}
                        <div className="grid grid-cols-4 gap-4">
                            <button onClick={() => handleSwitchImage('https://images.unsplash.com/photo-1549421263-50432c7457c4?q=80&w=1000&auto=format&fit=crop')} className="aspect-square bg-gray-100 rounded overflow-hidden border-2 border-black">
                                <img src="https://images.unsplash.com/photo-1549421263-50432c7457c4?q=80&w=1000&auto=format&fit=crop" alt="View 1" className="w-full h-full object-cover" />
                            </button>
                            <button onClick={() => handleSwitchImage('https://images.unsplash.com/photo-1586350977771-b3b0abd50c82?q=80&w=1000&auto=format&fit=crop')} className="aspect-square bg-gray-100 rounded overflow-hidden border-2 border-transparent hover:border-black transition-colors">
                                <img src="https://images.unsplash.com/photo-1586350977771-b3b0abd50c82?q=80&w=1000&auto=format&fit=crop" alt="View 2" className="w-full h-full object-cover" />
                            </button>
                            <button onClick={() => handleSwitchImage('https://images.unsplash.com/photo-1560769680-ba4b325651e8?q=80&w=1000&auto=format&fit=crop')} className="aspect-square bg-gray-100 rounded overflow-hidden border-2 border-transparent hover:border-black transition-colors">
                                <img src="https://images.unsplash.com/photo-1560769680-ba4b325651e8?q=80&w=1000&auto=format&fit=crop" alt="View 3" className="w-full h-full object-cover" />
                            </button>
                            <button onClick={() => handleSwitchImage('https://images.unsplash.com/photo-1603252109303-2751441dd157?q=80&w=1000&auto=format&fit=crop')} className="aspect-square bg-gray-100 rounded overflow-hidden border-2 border-transparent hover:border-black transition-colors">
                                <img src="https://images.unsplash.com/photo-1603252109303-2751441dd157?q=80&w=1000&auto=format&fit=crop" alt="View 4" className="w-full h-full object-cover" />
                            </button>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <h1 className="font-display text-4xl md:text-5xl font-medium tracking-tighter uppercase mb-4">Tournament Socks</h1>
                        <p className="text-2xl font-bold mb-8">$20.00</p>

                        <div className="mb-8">
                            <p className="text-gray-700 leading-relaxed mb-4">
                                Performance crew socks designed for tournament play. Cushioned sole provides comfort and support, while arch compression ensures a secure fit. Breathable mesh panels keep feet cool during intense matches.
                            </p>
                            <ul className="space-y-2 text-sm text-gray-700">
                                <li className="flex items-start gap-2"><i className="ph-fill ph-check-circle text-black mt-0.5"></i> Cushioned sole for comfort</li>
                                <li className="flex items-start gap-2"><i className="ph-fill ph-check-circle text-black mt-0.5"></i> Arch compression support</li>
                                <li className="flex items-start gap-2"><i className="ph-fill ph-check-circle text-black mt-0.5"></i> Breathable mesh ventilation</li>
                                <li className="flex items-start gap-2"><i className="ph-fill ph-check-circle text-black mt-0.5"></i> NEVA stripe detail</li>
                            </ul>
                        </div>
                        ...
                        <div className="mb-8">
                            ...
                            <div className="flex gap-3">
                                <button onClick={() => handleSelectSize('S')} className={`size-btn px-6 py-3 border-2 font-medium uppercase text-sm transition-colors ${selectedSize === 'S' ? 'border-black bg-black text-white' : 'border-gray-300 hover:border-black'}`}>S (5-7)</button>
                                <button onClick={() => handleSelectSize('M')} className={`size-btn px-6 py-3 border-2 font-medium uppercase text-sm transition-colors ${selectedSize === 'M' ? 'border-black bg-black text-white' : 'border-gray-300 hover:border-black'}`}>M (8-10)</button>
                                <button onClick={() => handleSelectSize('L')} className={`size-btn px-6 py-3 border-2 font-medium uppercase text-sm transition-colors ${selectedSize === 'L' ? 'border-black bg-black text-white' : 'border-gray-300 hover:border-black'}`}>L (11-13)</button>
                            </div>
                        </div>

                        {/* Add to Cart */}
                        <button ref={addToCartButtonRef} onClick={() => handleAddToCart({id: 'socks', name: 'Tournament Socks', price: 20.00, image: 'https://images.unsplash.com/photo-1549421263-50432c7457c4?q=80&w=1000&auto=format&fit=crop'})} className={`w-full py-4 text-white uppercase font-bold text-sm tracking-wider rounded active:scale-95 transition-all flex items-center justify-center gap-2 mb-4 ${addToCartStatus === 'success' ? 'bg-green-600' : 'bg-black hover:bg-gray-800'}`}>
                            <i className={`ph text-xl ${addToCartStatus === 'success' ? 'ph-fill ph-check-circle' : 'ph ph-shopping-cart'}`}></i>
                            <span>{addToCartStatus === 'success' ? 'Added to Cart!' : 'Add to Cart'}</span>
                        </button>
