...
export default function EventChallengerSeriesPage() {
  ...
  useEffect(() => {
    const fetchEventData = async () => {
      try {
        ...
        const challengerEvent = Array.isArray(eventsData) 
          ? eventsData.find(e => e?.slug === 'challenger-series' || e?.name?.includes('Challenger'))
          : null;
        
        if (challengerEvent) {
          setEvent(challengerEvent);
          
          const regsRes = await fetch(`/api/event-registrations?eventId=${challengerEvent.id}`);
  ...
  const handleEventRegistration = async (eventName) => {
    ...
    if (!isMember) {
      if (confirm('Club NEVA membership is required to register for events.\n\nWould you like to apply for membership now?')) {
        localStorage.setItem('returnToEvent', 'event_challenger_series');
      ...
      if (confirm(`Register for ${eventName}?\n\nYou'll be charged $${event?.price || 35} and receive a confirmation email.`)) {
        try {
          ...
          if (res.ok) {
            ...
            const regsRes = await fetch(`/api/event-registrations?eventId=${event.id}`);
            const regsData = await regsRes.json();
            setRegistrations(Array.isArray(regsData) ? regsData : []);
          }
        } catch (error) {
  ...
  const registeredCount = registrations?.length || 8;
  const capacity = event?.capacity || 20;
  const spotsLeft = event?.spots_left || (capacity - registeredCount);
  ...
  <main className="pt-20">
      {/* Event Hero */}
      <section className="relative h-[300px] md:h-[400px] bg-gradient-to-br from-blue-900 via-indigo-800 to-purple-900">
          <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1622279457486-62dcc4a431d6?w=1200')] bg-cover bg-center opacity-30"></div>
          <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
          <div className="relative h-full max-w-5xl mx-auto px-6 flex flex-col justify-end pb-12">
              <div className="flex items-center gap-2 mb-4">
                  <span className="px-3 py-1 bg-green-600 text-white text-xs font-bold uppercase tracking-widest rounded-full">{event?.status === 'upcoming' ? 'Open' : event?.status || 'Open'}</span>
                  <span className="px-3 py-1 bg-white/10 backdrop-blur text-white text-xs font-bold uppercase tracking-widest rounded-full">{event?.event_type || 'Round Robin'}</span>
              </div>
                  ...
                  <div className="md:col-span-2 space-y-8">
                      <div>
                          ...
                          <div className="prose prose-gray max-w-none text-gray-700 leading-relaxed">
                              <p>{event?.description || 'The Challenger Series is designed for intermediate to advanced players looking to sharpen their competitive edge. This weekly series offers a perfect balance of structured competition and skill development.'}</p>
                              <p className="mt-4">Format: Round robin doubles play with rotating partners. Perfect for players looking to expand their network while improving match play skills.</p>
                              <p className="mt-4"><strong>What's Included:</strong></p>
                              <ul className="list-disc pl-5 mt-2 space-y-1">
                                  <li>3 hours of guaranteed court time</li>
                                  <li>Professional-grade tournament balls</li>
                                  <li>Match stats and performance tracking</li>
                                  <li>Post-match networking session</li>
                      ...
                      <div className="border-t border-gray-200 pt-8">
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                              {Array.isArray(registrations) && registrations.length > 0 ? registrations.map((reg, idx) => {
                                const initials = reg?.member_name ? reg.member_name.split(' ').map(n => n[0]).join('') : 'CN';
                                const colors = [
                                ...
                                return (
                                  <div key={reg?.id || idx} className="flex items-center gap-2 p-3 bg-gray-50 rounded-lg">
                                      ...
                                      <div className="flex-1 min-w-0">
                                          <div className="font-medium text-sm truncate">{reg?.member_name || 'Member'}</div>
                                      </div>
                                  </div>
                                );
                              }) : (
                                <>
                                  <div className="flex items-center gap-2 p-3 bg-gray-50 rounded-lg">
                                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-cyan-600 flex items-center justify-center text-white font-bold text-sm">BW</div>
                                      <div className="flex-1 min-w-0">
                                          <div className="font-medium text-sm truncate">Brandon White</div>
                                      </div>
                                  </div>
                                  <div className="flex items-center gap-2 p-3 bg-gray-50 rounded-lg">
                                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-purple-500 to-pink-600 flex items-center justify-center text-white font-bold text-sm">JR</div>
                                      <div className="flex-1 min-w-0">
                                          <div className="font-medium text-sm truncate">Jessica Roberts</div>
                                      </div>
                                  </div>
                                  <div className="flex items-center gap-2 p-3 bg-gray-50 rounded-lg">
                                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-green-500 to-emerald-600 flex items-center justify-center text-white font-bold text-sm">CP</div>
                                      <div className="flex-1 min-w-0">
                                          <div className="font-medium text-sm truncate">Chris Parker</div>
                                      </div>
                                  </div>
                                  <div className="flex items-center gap-2 p-3 bg-gray-50 rounded-lg">
                                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-orange-500 to-red-600 flex items-center justify-center text-white font-bold text-sm">AH</div>
                                      <div className="flex-1 min-w-0">
                                          <div className="font-medium text-sm truncate">Ashley Hill</div>
                                      </div>
                                  </div>
                                  <div className="flex items-center gap-2 p-3 bg-gray-50 rounded-lg">
                                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold text-sm">MG</div>
                                      <div className="flex-1 min-w-0">
                                          <div className="font-medium text-sm truncate">Michael Green</div>
                                      </div>
                                  </div>
                                  <div className="flex items-center gap-2 p-3 bg-gray-50 rounded-lg">
                                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-yellow-500 to-orange-600 flex items-center justify-center text-white font-bold text-sm">SB</div>
                                      <div className="flex-1 min-w-0">
                                          <div className="font-medium text-sm truncate">Samantha Brown</div>
                                      </div>
                                  </div>
                                  <div className="flex items-center gap-2 p-3 bg-gray-50 rounded-lg">
                                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-pink-500 to-rose-600 flex items-center justify-center text-white font-bold text-sm">DA</div>
                                      <div className="flex-1 min-w-0">
                                          <div className="font-medium text-sm truncate">Daniel Adams</div>
                                      </div>
                                  </div>
                                  <div className="flex items-center gap-2 p-3 bg-gray-50 rounded-lg">
                                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-teal-500 to-cyan-600 flex items-center justify-center text-white font-bold text-sm">RJ</div>
                                      <div className="flex-1 min-w-0">
                                          <div className="font-medium text-sm truncate">Rachel Jones</div>
                                      </div>
                                  </div>
                                </>
                              )}
                      ...
                      <div className="sticky top-24 space-y-6">
                          ...
                          <div className="border border-gray-200 rounded-2xl p-6 bg-white shadow-sm">
                              <div className="flex items-baseline justify-between mb-6">
                                  <span className="text-3xl font-display font-bold">${event?.price || 35}</span>
                                  <span className="text-gray-500 text-sm">per player</span>
                              </div>
                              
                              <button onClick={() => handleEventRegistration(event?.name || 'Challenger Series')} className="w-full py-4 bg-black text-white font-bold uppercase text-sm tracking-widest rounded-lg hover:bg-gray-800 transition-all active:scale-95 mb-4">
                                  Register Now
                              </button>
                          ...
                          <div className="border border-gray-200 rounded-2xl p-6 bg-white">
                              ...
                              <p className="font-medium mb-2">{event?.location || 'Downtown Courts'}</p>
                              <p className="text-sm text-gray-600 mb-4">425 Piedmont Ave NE<br />Atlanta, GA 30308</p>
