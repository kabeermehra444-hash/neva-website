...
export function setStoredMember(memberObj) {
  ...
  // Never persist tokens inside the member object; keep them separate.
  const { adminToken, memberToken, ...clean } = memberObj || {};
  localStorage.setItem(KEYS.member, JSON.stringify(clean));
  if (adminToken) localStorage.setItem(KEYS.adminToken, adminToken);
  if (memberToken) localStorage.setItem(KEYS.memberToken, memberToken);
}
...
export function clearStoredMember() {
  ...
  localStorage.removeItem(KEYS.memberToken);
}
...
export function getMemberToken() {
  if (typeof window === 'undefined') return null;
  return localStorage.getItem(KEYS.memberToken);
}

// Returns headers to spread into a member-only fetch(), including the bearer token.
export function memberHeaders(extra = {}) {
